...
function doGet(e) {
  try {
    ...
    switch(action) {
      ...
      case 'login':
        if (!dataParam) {
          result = { error: 'Dados não fornecidos' };
        } else {
      ...
      case 'createRegistration':
        if (!dataParam) {
          ...
          const regData = JSON.parse(dataParam);
          result = createRegistration(regData);
        }
        break;
      ...
      case 'deleteRegistration':
        if (!dataParam) {
          ...
          const delData = JSON.parse(dataParam);
          result = deleteRegistration(delData.id);
        }
        break;
